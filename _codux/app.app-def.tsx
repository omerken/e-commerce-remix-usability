...
const manifestToRouter = (
    ...
    const rootFilePath = (manifest.homeRoute || manifest.routes[0])?.parentLayouts?.[0]
        ?.layoutModule;
    if (!rootFilePath) {
    ...
    if (manifest.homeRoute) {
        rootRoute.children = [
            fileToRoute(manifest.homeRoute.pageModule, requireModule, setUri, '/'),
        ];
    }
                ...
                if (value.path.length === 0) {
                    ...
                    const routeFileExports = await Promise.all(
                        routeFiles.map(async (file) => {
                            const fileExportsWatcher = fsApi.watchFileExports(
                                file,
                                (exports) => exports
                            );
